...
///
/// Every check reads the header alone. A file is recognised from the first
/// piece a stream handed over. The first number below is where a check can
/// answer. The second is where it has read every field it checks:
///
...
/// reached and can pass data that a whole header would fail. A tar written
/// before ustar carries no magic and needs all 512 bytes.
///
/// Six bytes answer for every format but tar, twelve run every check but the
/// tar checksum, and [headerBytes] decides all of it. A format with no header
/// of its own, raw LZMA and raw deflate among them, is not here
abstract final class CodecsRecognizer {
  /// The most bytes any of these checks reads, one tar header
  static const headerBytes = 512;

  /// `1f 8b`, then the compression method, fixed at eight. RFC 1952 reserves
  /// the top three bits of FLG and a decoder must refuse a member that sets
  /// them
  ...
  /// the two together are a multiple of thirty one. A header alone cannot
  /// settle it. This check gives a rare false positive
  ...
  /// `BZh` and the block size, one of nine hundred kilobyte steps. The magic is
  /// plain text. The six bytes after it are matched too when the data has them,
  /// the block magic or the end of stream magic
  ...
  /// `fd 37 7a 58 5a 00`, two bytes of stream flags, then their CRC32. Every
  /// stream header carries that CRC32. The first flag byte and the top four
  /// bits of the second must be zero
  ...
  /// A tar header is 512 bytes. The versions before ustar carry no magic and
  /// the checksum identifies those. The eight bytes it sits in count as spaces,
  /// and old writers summed them as signed. Short of a whole header the ustar
  /// magic still answers.
  ///
  /// A whole header with a bad checksum is not a tar, magic or no magic. `file`
  /// calls it data and libarchive's `archive_read_format_tar_bid` returns 0
  /// before it looks at the magic. Do not add a magic fallback here
  ...
  /// The format the data starts with, or [ArchiveFormat.unknown]. The order
  /// matters only for zlib. Its two byte header can be read out of another
  /// format's first bytes, and [withZLib] is off by default for that
  ...
  /// of a tar wrapped in the format, tar.bz2, tbz and so on
